...
Valid["stack"]      = Valid_stack; // Valid.stack(message:String = ""):String
//}@assert
    ...
    return Object.keys(scheme).every(function(schemeKey) {
        ...
        if (schemeKey in json) {
            if ( !Valid_type(json[schemeKey], schemeType) ) {
                console.error("JSON Scheme Error. Type missmatch: " + path + schemeKey + " is not " + schemeType);
                return false;
        ...
        console.error("JSON Scheme Error. Unknown property: " + path + schemeKey);
        return false;
    });
}
//}@assert
